 ...
 * - Foto del producto (comprimida a base64)
 * - Vista en grilla de tarjetas (estilo calendario)
 */

const STORAGE_KEY = "kiosco_stock_v3";
const THEME_KEY = "kiosco_theme";
const MAX_IMG_SIZE = 400; // px del lado mayor al comprimir
...
function cargarProductos() {
  try {
    ...
    // Migración suave desde versiones anteriores
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      ...
      img.onload = () => {
        ...
        // JPEG con calidad 0.72 para reducir tamaño
...
function actualizarFiltroCategorias() {
  ...
  if (cats.includes(actual)) select.value = actual;
...
function renderGrid() {
  ...
  grid.innerHTML = lista
    .map((p) => {
      ...
      const margen =
        compra > 0 ? Math.round(((venta - compra) / compra) * 100) : null;
